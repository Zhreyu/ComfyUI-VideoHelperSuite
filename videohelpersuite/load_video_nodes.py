...
def cv_frame_generator(
    ...
    # 1) build & resample to 24fps on disk
    base, ext = os.path.splitext(video)
    resampled_path = f"{base}_24fps{ext}"
    ...
    # from here on out, work off the resampled file...
    try:
        video_cap = cv2.VideoCapture(resampled_path)
        if not video_cap.isOpened() or not video_cap.grab():
            raise ValueError(f"{resampled_path} could not be loaded with cv.")
        ...
        target_frame_time = (1.0 / force_rate) if force_rate else base_frame_time

        # compute how many frames we’ll yield
        if total_frames > 0:
            if force_rate:
        ...
        # first yield: metadata tuple
        yield (width, height, fps, duration,
               total_frames, target_frame_time,
               yieldable_frames)
        ...
        while video_cap.isOpened():
            ...
            if frame_load_cap and (frames_added >= frame_load_cap):
                break

        # teardown meta_batch if used
